const { createDocRecordFromJobRecord } = require("./record_doc");
...
(async () => {
  const promises = posts.map(async (post) => {
    ...
    let jobRecords = await createJobRecordsFromcompanyRecord(companyRecord);

    jobRecords.forEach((jobPostRecord) => {
      console.log("jobPostRecord", jobPostRecord);
      console.log("createDocRecordFromJobRecord STARTING");
      let docRecord = createDocRecordFromJobRecord(jobPostRecord);
    });
  });
})();
